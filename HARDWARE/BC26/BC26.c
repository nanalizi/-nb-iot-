...
BC26 BC26_Status;
////阿里云的三元素,在设备里面都可以查询到//////////////////////
#define ProductKey     "a1pEJg7uaBv"             //产品KEY
#define DeviceName          "lizi"      //
#define DeviceSecret    "694502a1956a3faf8be0975118e6af3e"  //
#define PubTopic         "/sys/a1pEJg7uaBv/lizi/thing/event/property/post"//发布主题
#define SubTopic        "/sys/a1pEJg7uaBv/lizi/thing/service/property/set"//订阅主题
...
void aliyunMQTT_PUBdata(u8 temp,u8 PH,u8 TU)
{
     u8 t_payload[300],len;
     printf("AT+QMTPUB=0,0,0,0,%s\r\n",PubTopic);//发布主题
     delay_ms(300);
     len=Mqttaliyun_Savedata(t_payload,temp,PH,TU);
     t_payload[len]=0;
     printf("%s",t_payload);
...
//访问阿里云需要提交JSON数据
u8 Mqttaliyun_Savedata(u8 *t_payload,u8 temp,u8 PH,u8 TU)
{

    char json[]="{\"id\":\"26\",\"version\":\"1.0\",\"params\":{\"currentTemperature\":{\"value\":%d},\"turbidity\":{\"value\":%d},\"turbidity\":{\"value\":%d}},\"method\":\"thing.event.property.post\"}";	 //     char json[]="{\"datastreams\":[{\"id\":\"location\",\"datapoints\":[{\"value\":{\"lon\":%2.6f,\"lat\":%2.6f}}]}]}";
    char t_json[200];
    unsigned short json_len;
    sprintf(t_json, json, temp, PH, TU);
